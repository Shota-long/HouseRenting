...
public class CollectionDao {
	...
	public Object findCollection(int house_id,String username) {
	...
	public int addCollection(int house_id, String username) {
		//获取当前系统时间
		...
		Session session = HibernateUtil.getCurrentSession();
		String sql = "insert into collect_table(user_id,house_id,collect_time) select u.user_id,?,? from user_table u where u.username=?";
	...
	public int deleteCollection(int house_id, String username) {
		Session session = HibernateUtil.getCurrentSession();
		String sql = "DELETE c from user_table as u,collect_table as c WHERE u.user_id=c.user_id AND u.username=? AND c.house_id=?";
	...
	public List<House> findCollection(String username) {
		...
		Session session = HibernateUtil.getCurrentSession();
//		String hql = "select c.collect_time,h from User u , House h ,Collection c where u.uid = c.user_id and c.house_id = h.house_id and u.username = 'John'";
//		session.beginTransaction();
//		Query query = session.createQuery(hql);
//		query.setParameter("username",username);
//		Object result = query.getResultList();
		...
		for (Object[] object : result) {
			...
			house.setHouse_id((int) object[1]);
